<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select clearable style="width: 150px" class="filter-item" v-model="query.targetMarketcoinId" @change="handleFilter" placeholder="请选择币种">
        <el-option v-for="item in marketCoinList" :key="item.id" :label="item.code" :value="item.id">
        </el-option>
      </el-select>
      <!--<el-select clearable style="width: 150px" class="filter-item" v-model="query.template" @change="getList" placeholder="请选择模板名称">-->
    ...
    <el-table :data="list" v-loading="listLoading" border fit highlight-current-row
      ...
      <el-table-column align="center" label="模板名称" width="200" prop="template_name"></el-table-column>
      <el-table-column align="center" label="费率类型" width="100">
        <template slot-scope="scope">
          <span>{{scope.row.type | typeFilter}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="币币交易币种" width="150">
        <template slot-scope="scope">
          <span>{{scope.row.target_coin_code}}/{{scope.row.market_coin_code}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="费率" width="100" prop="fee_rate"></el-table-column>
      <el-table-column align="center" label="最大数量" width="100" prop="max_quantity"></el-table-column>
      ...
      <el-table-column align="center" label="创建时间" width="150" prop="create_time">
        <template slot-scope="scope">
          <span>{{scope.row.create_time | parseTime}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="修改人" width="100" prop="modifier_name"></el-table-column>
      <el-table-column align="center" label="修改时间" width="150" prop="modify_time">
        <template slot-scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :rules="rules" :inline="true" ref="dataForm" :model="tempInfo" label-position="left" label-width="170px" style='width: 900px; margin-left:50px;'>
        ...
        <el-form-item label="币币交易币种" prop="target_market_coin_id" class="item-right">
          <el-select clearable class="filter-item" v-model="tempInfo.target_market_coin_id" placeholder="请选择">
            <el-option v-for="item in marketCoinList" :key="item.id" :label="item.code" :value="item.id">
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="模板名称" prop="template" class="item-right">
    ...
    data() {
      return {
        ...
        rules: {
          ...
          target_market_coin_id: [{ required: true, message: '请填写', trigger: 'blur' }]
        },
        typeList: [{
          id: 1,
          name: '买'
