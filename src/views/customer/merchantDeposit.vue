<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button class="filter-item" style="margin-left: 10px;" @click="getList" type="primary" icon="el-icon-search">查询</el-button>
    </div>

    <el-table :data="list" v-loading="listLoading" border fit highlight-current-row
              style="width: 100%;min-height:500px;">
      <!--<el-table-column align="center" label="UID" width="100" prop="account_id"></el-table-column>-->
      <el-table-column align="center" label="邮件" width="170" prop="email"></el-table-column>
      ...
      <el-table-column align="center" label="商户等级" width="100" prop="merchant_level_id"></el-table-column>
      <el-table-column align="center" label="商户等级明细" width="120" prop="merchant_level_desc"></el-table-column>
      <el-table-column align="center" label="系统保证金额度限制" width="160">
        <template slot-scope="scope">
          <span>{{scope.row.system_deposit_limit}} {{scope.row.system_deposit_coin_code}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="商户保证金余额" width="160">
        <template slot-scope="scope">
          <span>{{scope.row.merchant_deposit_balance}} {{scope.row.system_deposit_coin_code}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="商户可用数字币资产余额" width="180">
        <template slot-scope="scope">
          <span>{{scope.row.merchant_available_balance}} {{scope.row.system_deposit_coin_code}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="商户冻结数字币资产余额" width="180">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" :label="$t('table.actions')" width="230" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="handleAction(scope.row, 'pay')">缴纳</el-button>
          <!--<el-button size="mini" type="warning" @click="handleAction(scope.row, 'return')">退回</el-button>-->
          <el-button size="mini" type="danger" @click="handleAction(scope.row, 'penalty')">扣罚</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    data() {
      return {
        ...
        rules: {
          ...
          info: [{ required: true, message: '请填写', trigger: 'blur' }]
        },
        updatingObj: ''
      }
    },
